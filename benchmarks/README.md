...
## v1.21.0 事件溯源崩溃恢复 + 退避抖动（PHP 8.3.33）

新增压测场景 `9`（FileEventStore，5 万事件）：写入后模拟崩溃（在文件末尾追加一条被截断的半行 JSON），全新实例重载时跳过损坏末行并重建出全部有效事件。结论：损坏行跳过几乎零开销，恢复路径与干净基线相当。

| 场景 | 耗时 | 说明 |
| --- | --- | --- |
| 写入 50,000 事件 | 1030.3 ms | 单次整行原子追加（FILE_APPEND \| LOCK_EX） |
| 重载恢复（跳过截断末行） | 41.4 ms | 损坏末行被跳过，恢复 50,000 条 |
| 重载（无损坏行基线） | 41.9 ms | 干净文件对照 |
